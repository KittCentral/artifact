...
public class FootpathsMaker : MonoBehaviour {
	...
	void Start () 
	{
		...
		for (int i = -3; i <= 3; i++) {
			for (int k = -3; k <= 3; k++) {
				...
				squaresInView[i+3][k+3].transform.position = new Vector3 ((cameraGridPosIntx + i)*2.56f, (cameraGridPosInty+k)*2.56f, Camera.main.transform.position.z + 4.98f );
				squaresInView[i+3][k+3].name = (cameraGridPosIntx+i).ToString() + " " + (cameraGridPosInty+k).ToString() + " footpaths";
	...
	private void loadRight(int posXInt, int posYInt)
	{
		...
		for (int i = -3; i <= 3; i++)//write over the far right column with the new squares
		{
			...
			squaresInView[6][i+3].transform.position = new Vector3 ((posXInt + 4)*2.56f, (posYInt+i)*2.56f, Camera.main.transform.position.z + 4.98f);
			squaresInView[6][i+3].name = (posXInt+4).ToString() + " " + (posYInt+i).ToString() + " footpaths";
	...
	private void loadLeft(int posXInt, int posYInt)
	{
		...
		for (int i = -3; i <= 3; i++)//write over the far left column with the new squares
		{
			...
			squaresInView[0][i+3].name = (posXInt-3).ToString() + " " + (posYInt+i).ToString() + " footpaths";
			squaresInView[0][i+3].transform.position = new Vector3 ((posXInt - 3)*2.56f, (posYInt+i)*2.56f, Camera.main.transform.position.z + 4.98f);
	...
	private void loadUp(int posXInt, int posYInt)
	{
		...
		for (int i = -3; i <= 3; i++)//fill the top row with the new squares
		{
			...
			squaresInView[i+3][6].name = (posXInt + i).ToString() + " " + (posYInt+3).ToString() + " footpaths";
			squaresInView[i+3][6].transform.position = new Vector3((posXInt+i)*2.56f,(posYInt+3)*2.56f,Camera.main.transform.position.z + 4.98f);
	...
	private void loadDown(int posXInt, int posYInt)
	{
		...
		for (int i = -3; i <= 3; i++) 
		{
			...
			squaresInView[i+3][0].name = (posXInt + i).ToString() + " " + (posYInt-3).ToString() + " footpaths";
			squaresInView[i+3][0].transform.position = new Vector3((posXInt+i)*2.56f,(posYInt-3)*2.56f,Camera.main.transform.position.z + 4.98f);
